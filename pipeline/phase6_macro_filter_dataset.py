...
def process_file(input_path, f_out, f_log):
    """Read one token file, group by video, filter, and write results."""
...
if __name__ == "__main__":
    ...
    if total_files == 0:
        print(f"❌ [Worker {task_id}] No files found matching pattern {args.input_pattern} in {args.input_dir}")
        raise SystemExit(0)
    ...
    print(f"🚀 [Worker {task_id}/{num_tasks}] Assigned {len(my_files)}/{total_files} files to process.")
    ...
    with open(out_clean_path, "w", encoding="utf-8") as f_out, open(out_log_path, "w", encoding="utf-8") as f_log:
        if total_assigned == 0:
            print(f"⚠️ [Worker {task_id}] No files assigned to this worker.")
        else:
            for idx, file_path in enumerate(my_files, start=1):
                ...
                print(
                    ...
                    f"{progress:.1f}% | Clean: {total_clean} | Discarded: {total_discarded}",
    ...
    print(f"✅ [Worker {task_id}] DONE! Videos passed: {total_clean} | Discarded: {total_discarded}")
    print(f"   📁 Clean data: {out_clean_path}")
