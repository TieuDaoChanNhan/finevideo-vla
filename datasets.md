...
**⚠️ Cập nhật scope (20/07/2026):** trước đây file này chủ yếu lọc dataset theo tiêu chí "có video/pose/action không, có khớp pipeline VLA-humanoid không". Huu (leader) xác nhận trực tiếp trong chat: dự án thực ra là **omni-modal** — bind bất kỳ tổ hợp modal nào (ảnh, video, âm thanh, action, IMU...), miễn **license permissive + cân bằng tỷ trọng modal + tạo được cross-modal binding thật** — không bắt buộc phải liên quan robot/humanoid. 2 dataset mới thêm dưới đây (`synth_llava`/`synth_llava2`, `laion/emotional-roleplay-finetuning-dataset`) đúng theo tiêu chí mới này — không có video/pose/action nào cả, nhưng vẫn hợp lệ vì là ảnh+caption / audio+text permissive.
...
| **synth_llava / synth_llava2** (2 file trong `MixtureVitae-Backup/multimodal`, chưa peek đợt 9/7) | ✅ Có (56.2GB, giải nén xong 55GB tại `$DATA/mixturevitae_multimodal/synth_llava/`) | ✅ seed2 (ảnh tĩnh, không cần vocab mới) | ⚠️ Chưa viết script tokenize, mới inspect cấu trúc | Huu tự tạo dataset này — 603,999 sample ảnh+caption tổng hợp (llava_pretrain style), **không có video/action**. Xem `REPORT.md` "Project Scope Update" 20/7 |
| **laion/emotional-roleplay-finetuning-dataset** | ✅ Có (2.5GB, `/p/data1/mmlaion/shared/vla/laion_emotional_roleplay/`, parquet) | ✅ snac (audio, đã có sẵn trong project) hoặc "moss" (chưa rõ, theo chỉ đạo Huu) | ❌ Chưa tokenize | 67,491 clip TTS tổng hợp (MOSS-TTS), text+voice_description+audio. License cc-by-4.0, public. Chỉ đạo Huu: "concatenate the text and interleave with snac and/or moss tokens" |
