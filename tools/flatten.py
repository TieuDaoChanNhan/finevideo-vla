"""
Flatten the merged XYZT dataset into Megatron-LM JSONL.

Reads final_dataset_xyzt (hierarchical scenes/activities) and produces
...
Token flattening:
    ...
    <agent> <fps_30> <joint_0_x_128> ... </agent>       → <fps_30> <joint_0_x_128> ...

Input:  /e/data1/.../FineVideo-VLA/final_dataset_xyzt/final_vla_xyzt_rank_*.jsonl
Output: /e/data1/.../nguyen38/flat_xyzt/flat_final_vla_xyzt_rank_*.jsonl

Usage (SLURM array):
    SLURM_ARRAY_TASK_ID=1 SLURM_ARRAY_TASK_COUNT=40 \\
        python tools/flatten.py [--drop_avc 1.0] [--drop_cosmos 1.0] [--drop_seed 1.0]
"""
...
def process_tokens_to_individual_tags(token_str, drop_rate_avc=1.0, drop_rate_cosmos=0.5, drop_rate_seed=0.5):
    ...
    XYZT agent blocks:    <agent> <fps_30> <joint_0_x_N> ... </agent> → kept as-is
    """
...
def main():
    parser = argparse.ArgumentParser(description="Flatten XYZT merged dataset into Megatron-LM JSONL.")
    parser.add_argument("--input-glob",
                        default="/e/data1/datasets/playground/mmlaion/shared/nguyen38/FineVideo-VLA/final_dataset_xyzt/final_vla_xyzt_rank_*.jsonl")
