...
class KinematicPreprocessor:
    ...
    def detect_hallucinations(self, pose_3d):
        ...
        # Tắt cảnh báo All-NaN slice an toàn
    ...
    def temporal_smooth(self, pose, window_size=5):
        """Dùng Moving Average Window để 'là phẳng' nhiễu tần số cao, an toàn với NaN (NaN-safe)."""
    ...
    def process(self, raw_pose3d):
        ...
        if np.isnan(clip_median_span) or clip_median_span == 0:
            print(f"🪓 Geometric Hallucination Filter: {np.sum(geometric_hallucination_mask)} / {len(pose)} frames bị chém bay.")
...
def apply_2d_mask(pose3d, json_2d_path):
    ...
    # 2D Gate: Từ điển phiên dịch COCO (HRNet) -> Human3.6M (MotionBERT)
    coco_to_h36m = {
        0: 9,   # Nose
        5: 11,  # L_Shoulder
        6: 14,  # R_Shoulder
    ...
    # Chỉ đánh NaN đúng vào khớp tương ứng, các khớp nội suy (như Pelvis) để yên
...
def process_file(input_path, output_path, processor, video_id, json_2d_dir):
    ...
    # BƯỚC 1: Lấp các lỗ hổng tracking tự nhiên CỦA MẢNG GỐC trước khi làm gì khác
    pose3d_interp = interpolate_nan_gaps(pose3d, max_gap=5)
    
    # BƯỚC 2: Áp dụng Máy Chém 2D Gate (Che chân/tay bị lấp)
    pose3d_masked = apply_2d_mask(pose3d_interp, json_2d_path)

    # BƯỚC 3: Đưa qua Lò Nắn Xương & Động Học
    pose_final = processor.process(pose3d_masked)

    # BƯỚC 4: Cắt cửa sổ an toàn
