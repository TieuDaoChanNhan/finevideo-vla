...
def compute_global_xy_projection(pose_3d, width, height):
    """Chiếu tọa độ 3D mét sang 2D pixel, fit theo chiều cao và đặt vào giữa màn hình."""
    
    pose = np.asarray(pose_3d, dtype=np.float64)
    finite_joint_mask = np.all(np.isfinite(pose), axis=-1)
    
    # 1. Lấy tọa độ Y (trục dọc) hợp lệ toàn cục để tính scale
    ...
    # 2. TÍNH SCALE: Ép chiều cao bộ xương chiếm 90% chiều cao video
    # Dùng chiều cao làm limiting factor
    scale = (height * 0.9) / y_range_global

    # 3. TÍNH TRUNG TÂM TOÀN CỤC (để đưa về gốc 0,0)
    xy = pose[:, :, :2].copy() # Lấy XY
    ...
    # 4. CHUYỂN ĐỔI: Đưa về gốc -> Scale -> Đưa về giữa màn hình
    xy_centered = xy - origin_center_global[None, None, :]
    xy_pix = xy_centered * scale 
    
    # Đưa image origin (0,0) từ trung tâm ra giữa màn hình
    x_pix = xy_pix[:, :, 0] + width / 2
    y_pix = xy_pix[:, :, 1] + height / 2
    
    # --- PHẦN VÁ LỖI LỘN NGƯỢC ---
    # CHẮC CHẮN CẬU ĐÃ XÓA HOẶC COMMENT DÒNG NÀY RỒI NHÉ:
    # y_pix = (height - 1) - y_pix 
    # -----------------------------
