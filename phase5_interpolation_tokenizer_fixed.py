...
class AdaptiveInterpolationTokenizer:
    ...
    def compute_arc_length_param(self, chunk_positions):
        ...
        # FIX: Ép quãng đường luôn nhích lên một chút (1e-5) để thời gian không bị đóng băng
    ...
    def reconstruct(self, cp, t_cp):
        ...
        # BẢO VỆ: Nếu token đã lưu có điểm thời gian bị trùng, ta cưỡng chế nhích nó lên 1e-5
...
def process_state_file(input_path, output_path, tokenizer, video_id, stride=16):
    """
    Đọc file *_cleaned.jsonl, lọc trùng lặp bằng stride, và nén thành token.
    """
    valid_chunks = 0
...
if __name__ == "__main__":
    ...
    if total_files == 0:
        print(f"❌ [Worker {task_id}] Không tìm thấy file *_cleaned.jsonl nào trong {args.input_dir}")
        raise SystemExit(0)
    ...
    print(
        f"\n🎉 [Worker {task_id}] HOÀN THÀNH! Đã nén thành công {total_tokens_generated} tokens "
        f"từ {processed} files (Skipped {skipped})."
    )
