...
#SBATCH --output=logs/merge_v3_%a.out
#SBATCH --error=logs/merge_v3_%a.err
...
OUT_DIR="${DATA}/final_dataset_adaptive_v3"

mkdir -p logs "${OUT_DIR}"

echo "[Worker ${SLURM_ARRAY_TASK_ID}/${SLURM_ARRAY_TASK_COUNT}] Starting merge v3 (has_seed2/has_cosmos chunk_timing fix)..."
...
python -u pipeline_pose/phase6_merge_adaptive.py \
  ...
  --output-dir       "${OUT_DIR}" \
  --output-prefix    "final_vla_adaptive_v3" \
  --skip-existing
