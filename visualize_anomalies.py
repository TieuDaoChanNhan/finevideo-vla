#!/usr/bin/env python3
"""
Visualize anomaly spans from V-JEPA filtering results.

This script reads:
- a real video
- a skeleton video
- a JSON result file produced by vjepa_filter.py

It then:
1) loads all clip-level anomaly predictions
2) merges overlapping / consecutive anomaly clips into larger anomaly spans
3) extracts the corresponding frames from both source videos
4) renders a side-by-side highlight reel:
      [ REAL VIDEO | SKELETON VIDEO ]
   with text overlay:
      - current frame index
      - mean cosine similarity for the anomaly span

Output:
- anomalies_highlight.mp4
...
python visualize_anomalies.py \
  ...
  --output anomalies_highlight.mp4
"""
...
def parse_anomaly_clips(data: Dict[str, Any]) -> List[ClipScore]:
...
def merge_anomaly_clips(clips: Sequence[ClipScore]) -> List[AnomalySpan]:
    if not clips:
        return []

    merged: List[AnomalySpan] = []
    cur_start = clips[0].start_frame
    cur_end = clips[0].end_frame_inclusive
    cur_sims = [clips[0].similarity]
    cur_clip_indices = [clips[0].clip_index]
...
def render_anomaly_highlight(
    video_real: str,
    video_skeleton: str,
    spans: Sequence[AnomalySpan],
    output_path: str,
    output_fps: Optional[float] = None,
    gap_frames: int = 8,
) -> None:
    if not spans:
        raise RuntimeError("No anomaly spans found. Nothing to render.")
        ...
        try:
            for span_idx, span in enumerate(spans):
                ...
                for frame_idx in range(start, end + 1):
                    ...
                    canvas = draw_text_block(
                        ...
                        [
                            ...
                            f"Anomaly span: {start}-{end}",
                            f"Mean cosine similarity: {span.mean_similarity:.4f}",
                            f"Merged clips: {span.num_clips}",
                        ],
                    )
                    writer.write(canvas)
                ...
                if gap_frames > 0 and span_idx < len(spans) - 1:
                    ...
                    gap = draw_text_block(
                        gap,
                        [f"Next anomaly span: {spans[span_idx + 1].start_frame}-{spans[span_idx + 1].end_frame_inclusive}"],
                    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Visualize anomaly clips from a V-JEPA result JSON.")
    ...
    parser.add_argument("--output", type=str, default="anomalies_highlight.mp4", help="Output MP4 filename.")
    parser.add_argument("--output-fps", type=float, default=None, help="Optional output FPS override.")
    parser.add_argument("--gap-frames", type=int, default=8, help="Black separator frames between anomaly spans.")
    return parser
...
def main() -> None:
    ...
    anomaly_clips = parse_anomaly_clips(result_data)
    spans = merge_anomaly_clips(anomaly_clips)
    ...
    print(f"anomaly clips    : {len(anomaly_clips)}")
    print(f"merged spans     : {len(spans)}")
    for i, span in enumerate(spans):
        print(
            f"[{i:03d}] frames {span.start_frame}-{span.end_frame_inclusive} | "
            f"mean_sim={span.mean_similarity:.4f} | merged_clips={span.num_clips}"
        )

    render_anomaly_highlight(
    ...
    print(f"\nSaved anomaly highlight reel to: {args.output}")
