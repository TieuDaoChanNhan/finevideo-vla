...
# IMPORTS FOR MMDet 1.x / MMPose 3.x
...
def process_video_to_json(video_path, output_json_path):
    ...
    while True:
        ...
        # SWITCH 1: Use MMDet to detect persons
        ...
        if len(bboxes) > 0:
            ...
            # SWITCH 2: Use MMPose to run pose estimation
...
if __name__ == "__main__":
    ...
    # Parser to receive offset from srun command
    parser = argparse.ArgumentParser()
    parser.add_argument('--offset', type=int, default=0)
    parser.add_argument('--total_workers', type=int, default=160)
    args = parser.parse_known_args()[0]

    # Compute global task ID from per-job offset
    ...
    task_id = global_task_id  # alias used in print statements

    # 1. CREATE A SEPARATE TEMP DIRECTORY FOR EACH WORKER
    ...
    # Partition videos by global rank
    my_ids = set([vid for i, vid in enumerate(all_ids) if i % total_global_tasks == global_task_id])

    print(f"🚀 [Global Worker {task_id}/{total_global_tasks}] Assigned {len(my_ids)} videos...")
    ...
    for item in dataset:
        ...
        if vid_id in my_ids:
            ...
            # 2. PLACE TEMP FILES IN THE WORKER'S OWN DIRECTORY
            ...
            try:
                # 1. Write raw bytes to mp4 in the worker's temp directory
                with open(tmp_mp4, "wb") as f:
                    f.write(video_bytes)

                # 2. Process and write to a TEMP JSON file in the worker's directory
                process_video_to_json(tmp_mp4, tmp_json_2d)

                # 3. ATOMIC RENAME: Move completed JSON to the shared OUT_2D directory
                if os.path.exists(tmp_json_2d):
                    os.rename(tmp_json_2d, final_json_2d)

                print(f"✅ [Worker {task_id}] 2D pose estimation done: {vid_id}")

            except Exception as e:
                print(f"❌ [Worker {task_id}] Error {vid_id}: {e}")
                if os.path.exists(tmp_json_2d):
                    os.remove(tmp_json_2d)  # delete stale temp JSON
            finally:
                if os.path.exists(tmp_mp4):
                    os.remove(tmp_mp4)  # clean up temp MP4

    # 3. CLEAN UP TEMP DIRECTORY AFTER ALL WORK IS DONE
