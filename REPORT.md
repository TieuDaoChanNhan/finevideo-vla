...
## 32. Seed2 Decoder Fixed (3 Real Bugs), SNAC/Seed2 Decoders Hardened Against Crashes, L2 SNAC Vocab Expansion Attempted Then Reverted (Dataset Has No L2 Data), Full Media-Eval Framework Built, Greedy Macro-Repetition Discovered in Both `seed2` and `cosmos`, Discord Analysis With Huu Yields Corrected Hypotheses + Roadmap (Jul 22, 2026, continued)

**Context:** Continuation of the same day as §31 (training confirmed complete, first cosmos decode). This entry covers a long follow-on session of decoder debugging, eval-framework building, and cross-checking findings against a real Discord conversation with Huu (PI) about the same results.

### 1. L2 SNAC vocab expansion — added to both tokenizers, then fully reverted

Wrote `tools/tokenizer/add_snac_l2_tokens.py` to add the never-encoded SNAC level-2 (50Hz, finest) band — 16,384 new ids (`<snac_148746>`–`<snac_165129>`) — to scratch copies of both `tokenizer_vla_qwen3` (→`tokenizer_vla_qwen3_snacfull`, 257,901→274,285 vocab) and `tokenizer_vla_adaptive` (→`tokenizer_vla_adaptive_snacfull`, 144,215→160,599 vocab), motivated by an atomicity test that appeared to find a bug at `<snac_140553>`. That "bug" was itself later retracted (§31 follow-up): the id fell in a real, intentionally-unallocated gap between the L0/L1 bands, not a defect. Per direct user instruction ("datasets chúng ta làm gì có L2 kia đâu" — our datasets don't have any L2 data at all), **both expanded tokenizer copies and the script itself were deleted** — the canonical tokenizers were never touched (script always wrote to new output dirs), so this had zero effect on the trained `qwen3_1.7b_vla_v2` checkpoint. Re-confirmed the atomicity test's original spot-check list in `tools/eval/eval_vla_v2_sanity.py` used a bad hand-picked id (`<snac_140553>`, landing in the unallocated `136458–144649` gap) and replaced it with real first/last ids of each of the 3 actual SNAC bands (L0, L1-even, L1-odd) — re-ran atomicity against the canonical (unmodified) tokenizer: **41/41 PASS**.

### 2. `decode_seed2.py`: 3 real bugs found and fixed, verified end-to-end

Retrying the seed2 decode that failed in §31 ("no decoder exists for this direction") surfaced three distinct, unrelated bugs, each fixed and regression-tested:

1. **`stabilityai/stable-diffusion-2-1-unclip` genuinely removed from HuggingFace** (confirmed via the actual page title, literally "404 – Hugging Face", not a gated-access page; confirmed absent from the `stabilityai` org's live model list via the HF search API) — not a permissions/gating issue as first assumed. Switched `DIFFUSION_NAME` to the community mirror `sd2-community/stable-diffusion-2-1-unclip` (same weights/pipeline class, openrail++ license, created as a direct replacement after the original disappeared).
2. **Wrong `Seed2Tokenizer` class imported.** `_load_seed2_tokenizer()`'s import shim was copied verbatim from `data_prep/synth_llava/tokenize_seed2.py`, which only ever calls `.encode_image()` and so only needs `prototype/pipeline.py`'s thin wrapper class — but decode needs `.decode()`/`.from_pretrained()`, which only exist on the real `PreTrainedModel`-based class in `seed2_tokenizer.py` itself. Fixed to return `_seed2_tokenizer.Seed2Tokenizer` directly.
3. **Missing batch dimension.** `get_codebook_entry(indices)` in `seed2_tokenizer.py` has no batch handling of its own — a flat `(32,)` tensor was silently read as 32 separate 1-token images instead of 1 image made of 32 query tokens, desyncing from the diffusion pipeline's own batch/CFG-doubling assumption deep in the UNet (`RuntimeError: tensor a (2) must match tensor b (64)`). Fixed with `.unsqueeze(0)`.
4. (Minor, same pass) **Output path resolved after `os.chdir()`.** `_load_seed2_tokenizer()` changes cwd to `PROTOTYPE_DIR` for its own relative lookups; a relative `--output` was silently landing there instead of the intended directory. Fixed by resolving `os.path.abspath(args.output)` before any chdir happens.

After all four fixes, successfully decoded the 32 real ground-truth `<seed2_N>` tokens from the §31 `synth_llava2_003266024` caption test to a real PNG (`seed2_decoded_groundtruth_boy_graduation.png`) — first confirmed `seed2`→image round-trip in this repo, closing the last "no decoder exists" gap from §31.

**Enhancement:** `decode_seed2.py` now also supports text containing *multiple* `<seed2>...</seed2>` blocks — each is decoded to its own numbered output (`_0`, `_1`, ...) instead of concatenating all ids and erroring, since `Seed2Tokenizer`'s Q-former only ever handles exactly 32 tokens (1 image) at a time.

### 3. Hardened `decode_snac.py` and `decode_seed2.py` against crashes on malformed input

Building the media-eval framework (below) surfaced a real crash: a sampled generation's snac span, extracted via `decode_snac.py`'s whole-text fallback (used when no `<snac>...</snac>` wrapper is present), spliced together token ids from non-contiguous positions in the output — breaking the strict positional L0→L1a→L1b assumption and producing an out-of-`[0,4096)` codebook index, which SNAC's GPU embedding lookup turned into an opaque `CUDA error: device-side assert triggered` instead of a real error. Root-caused via manual band-boundary arithmetic and confirmed the exact triplet indices at fault. Added explicit range validation before any tensor/GPU work in both scripts:
- `decode_snac.py`: validates each triplet's 3 raw indices are in `[0, 4096)` before use, raising a clear `ValueError` naming the offending triplet/position/token instead of crashing.
- `decode_seed2.py`: validates token count is exactly `SEED2_QUERY_LEN=32` before use (see the earlier concatenation bug), same clear-error treatment.

Both regression-tested against the exact scenarios that previously crashed — both now fail with a legible one-line Python error instead of a CUDA crash.

### 4. Built `tools/eval/eval_vla_v2_media.py` — full media-eval framework

A single script that: generates from a fixed prompt suite (spanning **continuation** — real ground-truth tokens fed in, e.g. the existing `image_caption`/`agent_continuation`/`roleplay_speech` tests; **from-scratch** — bare open tag or text-only, model invents everything; **full_chain** — one long open text prompt, model must walk all modality transitions unaided) under both greedy and sampling decoding, then auto-detects and decodes every modality present in each output via the now-fixed/hardened decoders, writing one `input_output.txt` (full prompt + full raw generation, nothing truncated) plus all decoded media per test into its own folder, with a `SUMMARY.md` linking everything. Supports `--only <name-substring>` and `--modes greedy,sample` for targeted reruns, and a per-prompt `max_new_tokens` (bumped `full_chain_from_scratch` to 4000 — the practical ceiling, since `config.json`'s `max_position_embeddings=4096` is a hard cap this checkpoint was trained under).

First full 14-test run (`samples/qwen3_1.7b_vla_v2_eval/2026-07-22_full_eval/`): 27 media files decoded successfully. Notable structural findings from this run alone (before the deeper full_chain investigation below): seed2 decode legitimately fails (not a bug) whenever a text span isn't exactly 32 tokens (e.g. a 3-token partial-context prompt, or a 96-token/3-block full_chain span) — this is what led to the multi-block-decode enhancement in §2 above.

### 5. Deep dive on `full_chain_from_scratch`: greedy macro-repetition discovered in BOTH `seed2` and `cosmos`

At 2000 tokens (the original test), the model never opened `<agent>` at all — working theory was "cosmos ate the budget" (787/2000 tokens on cosmos alone). Doubling to 4000 tokens (near the hard ceiling) **did not change this** — still zero `<fps_N>` tokens, ruling out "just needed more room." What the extra budget actually produced: the same modality cycle repeated roughly twice as many times (8 cosmos blocks vs 3, 160 seed2 tokens vs 96, 150 snac tokens vs 75) — never once visiting agent.

Two further checks nailed down the real mechanism:
- **All 5 `<seed2>...</seed2>` blocks generated across the 4000-token run are byte-identical** (same 32 ids, same order, every time).
- **Cosmos chunks are also near-identical across re-openings within one generation**: for this run, only 2/8 chunks were byte-unique (chunk 0 differs, chunks 1–7 are one repeated chunk); for the earlier skateboard/walking novel-prompt runs (see below), only 1/4 and 2/8 chunks were unique respectively. Practical consequence: an "8 chunks decoded" summary line represents at most ~2 distinct 8-frame (~0.27s) snippets of real content, not 8 sequentially-progressing windows — `max_new_tokens` under greedy buys more *repeats*, not more *unique seconds* of video.

Mechanism (analysis, not yet independently verified against the model internals): greedy is a deterministic function of context. Once the interleaved modality cycle (caption→seed2→snac→speech→cosmos→snac→...) settles into a repeating pattern, the local context immediately preceding each re-entry into `<seed2>`/`<cosmos>` becomes near-identical to its previous occurrence, and greedy — having no source of randomness — reproduces the same highest-probability continuation each time. This is the same class of failure long documented in NLG literature as greedy/argmax repetition loops, not unique to this project, though see the Discord-derived correction in §7 below re: whether "8-frame windows are near-static in the training data itself" is a contributing structural cause rather than purely a decoding-time artifact.

### 6. Novel-prompt test disproves prompt memorization; agent-reachability is prompt-dependent, not universal

To rule out the `full_chain_from_scratch` prompt ("Home cooking tutorial... chops vegetables...") being a verbatim/near-verbatim training example (git history shows it was hand-written to match the format convention, with no "REAL record" citation unlike other prompts in the same file that are explicitly labeled — but this wasn't independently verified against the actual ~40,000-record training corpus), ran the same Title/Context/Keywords format with a deliberately unrelated topic (skateboarding), greedy, 4000 tokens (`tools/eval/gen_full_chain_novel_prompt.py`):

- **Caption/speech content correctly tracked the new topic** ("The person is skateboarding on a sidewalk" / "I'm going to go to the beach" — nothing carried over from the cooking prompt's wording), directly refuting simple verbatim memorization.
- **Agent WAS reached this time** — 7 `<fps_30>` occurrences, successfully decoded to `decoded_agent_pose.mp4`. But per PI feedback (§7), the pose is completely static: measured pelvis movement range is exactly `[0, 0, 0]` across all 3 axes in all 7 windows.
- A third prompt ("A man walks down a street", per PI's specific request, §7) landed back in the no-agent pattern (0 `<fps_N>` tokens) despite correct topical caption ("walking down the sidewalk").
- Cross-checked whether `cosmos` content itself is topic-conditioned or generic: first-chunk token overlap is 0/200 between cooking and skateboard (completely different scenes), but 28/200 between skateboard and man-walking (both outdoor/pedestrian scenes, including identical first 3 ids) — consistent with real (if coarse) topic-conditioning at the single-chunk level, isolating the "no progression" problem to the *repetition* dynamic described in §5, not a failure to condition on the prompt at all.

Conclusion: agent-reachability under greedy is prompt-specific (varies unpredictably: skateboard reached it, cooking and walking did not), not a uniform "model can't do agent" limitation, and not evidence of dataset memorization.

### 7. Cross-checked against a real Discord conversation with Huu (PI) — corrections and new leads

User shared the actual Discord thread discussing these same skateboard/walking results with Huu. Cross-referencing surfaced:

- **Correction to this session's own working hypothesis:** earlier analysis (in this document, this same day) attributed cosmos-token-cost dominance partly to the training-time modality dropout numbers documented in `CLAUDE.md` (99% avclm / 90% cosmos) — but those figures describe the **first (v1) model's** training config. Van Khue's own words in the Discord thread for this v2 model: *"I just randomly trained, didn't do token dropout"* — v2 was trained with **no modality dropout at all**. The dropout-based explanation for v1's imbalance does not carry over to v2's cosmos dominance, which is purely a per-chunk-token-cost effect (200 tokens/chunk vs ~1-4 for other modalities) with no data-exclusion involved.
- **New structural hypothesis for the "no movement" finding, independent of the greedy-repetition mechanism in §5:** Huu, reasoning from first principles about the pipeline (*"8 frames might be way too slow"* → *"so most videos are like no movement, you see what i mean"*, confirmed by Van Khue): an 8-frame chunk at native 30fps is ~0.27 seconds — plausibly too short a window for many human actions to show visible motion **even in the real ground-truth training data**, since every video is chunked into fixed 8-frame windows regardless of its actual pacing (confirmed: *"currently no [variable-length windows], it will just gen a chunk of 8 frames... for every video"*). If true, the model's near-static default may partly reflect a genuine property of the training data at this granularity, not purely a decoding-time repetition artifact. Proposed fix (not yet implemented): speed up/resample source video before chunking so each 8-frame window spans more real elapsed time and thus more visible motion.
- **Two open data-integrity questions raised by Huu, not yet investigated:** (1) whether cosmos tokens are ever mismatched/misaligned in the pretrain data — prompted by a test where seed2→cosmos generation for one scene produced what looked like content from an unrelated "cutting/finger" scene; (2) whether cosmos generation is being silently truncated by the 4,096-token context limit mid-chunk. Both remain open follow-ups, not confirmed either way.
- **Roadmap items from the same thread, not yet started:** rebalance/reduce cosmos token-dropout for the next training run (already anticipated, see §31's `qwen3_1.7b_vla_v3.yaml`); scale to ~100B tokens for the run after that, with a large new majority-text component (proposed split: 70B general text from MixtureVitae-v1 + 30B from the current VLA-flavored mix — a substantial shift from the current ~32B all-VLA-flavored mix); systematic "any-modality-to-any-modality" testing (feed real cosmos/seed2 tokens of a scene, check other modalities complete correctly) — only done ad hoc so far, not systematically; test against real records pulled directly from the pretrain corpus (e.g. a real synth-llava "chemical bond" image, a real speech continuation) rather than only from-scratch/synthetic prompts; a longer-term idea for chain-of-thought-style cross-modal reasoning data (`<think>` blocks containing imagined intermediate seed2/cosmos content before a final action), generated "backwards" from real videos — no design or implementation started; distinguishing a coarser "SNAC listen" format (current, 2-of-3 codebook levels) from a full-fidelity "SNAC speak" format (all 3 levels) for better emotional-speech quality — directly relevant to the L2 SNAC work in §1 that was built then reverted this session; hosting an inference endpoint; explicit instruction that **"we need to work on evals now"**, which `eval_vla_v2_media.py` (§4) is a direct, if partial, answer to.

### 8. Housekeeping: `samples/qwen3_1.7b_vla_v2_eval/` cleanup, and a mid-session data-loss incident

Removed 3 flat files that were either superseded (`decode_seed2.log`, the original 404-failure log, since replaced by a successful retry log) or unverifiable (`snac_decoded_REAL_groundtruth_compare.wav`, which had no accompanying log/command recording how it was produced — replaced with a freshly-generated, fully-logged, verified pair: a real ground-truth mp3 pulled directly from the source parquet, decoded alongside its own round-tripped SNAC reconstruction).

**Separately, and not yet root-caused:** partway through this session, all loose files directly inside `samples/qwen3_1.7b_vla_v2_eval/` (both the 8 files already committed in §31, and ~8 more created earlier in this session) were found missing from disk — subdirectories were untouched. No `rm`/deletion code was found in any script written this session, and shell history for this session shows no matching command. The 8 previously-committed files were trivially restored via `git checkout HEAD -- <paths>`. The ~8 uncommitted files created earlier this session (including the first successful `seed2_decoded_groundtruth_boy_graduation.png` from §2, the verified real-vs-decoded SNAC comparison pair, and the from-scratch seed2 generation sample) are genuinely gone from disk and were **not** regenerated before this entry was written (all are reproducible from commands/records already documented above, but doing so was deprioritized this session in favor of wrapping up) — copies of all of them were already sent to the user directly during the session via chat, so the content is not entirely lost, just absent from the repo working tree. Flagging this explicitly rather than silently omitting it from the record.

### Status at end of entry

- `qwen3_1.7b_vla_v2` training + all decoders (`seed2`, `cosmos`, `snac`, `agent`): all verified working end-to-end against real and self-generated tokens.
- Media-eval framework (`eval_vla_v2_media.py` + 2 supporting one-off scripts) built and used for ~20 generations across 3 different runs.
- Greedy macro-repetition (whole-block, not just single-token) confirmed as a real, reproducible phenomenon across both `seed2` and `cosmos`, with 2 candidate contributing mechanisms identified (decoding-time repetition loop; possibly-too-short 8-frame training granularity) — not yet disentangled from each other, sampling-vs-greedy on these specific full_chain prompts not yet tested.
...
## 36. Phase 1 Follow-Up: Real-Video Resolution A/B, SNAC L2 A/B (+ MV-Omni Constraint Found), Cosmos Square-Crop Removed (Aspect-Preserving Now), Frame-Stride ("Speed-Up") Experiment -- All With Real Media Sent to User (Jul 22, 2026, continued)

**Context:** Direct follow-up to §35 in the same session. User asked 3 things: (1) verify empirically whether cosmos was "already 512 tokens" as remembered; test several resolutions on a real short video and decode each back so the user can visually pick one; (2) test SNAC with L2 added on a few real records, decode both versions, so the user can A/B by ear; (3) recommend a seq_length number and confirm scope is video-pipeline-first (cosmos+snac retokenize before seed2/pose), then analyze the 8-frame-window "no movement" question in depth. A follow-up message added: the user noticed cosmos is being **square**-cropped and asked whether that's still appropriate now that the project is omni (not robot-only, where the original square choice came from a jittery, roughly-square robot camera).

### 1. Real-video resolution A/B -- confirms exact quadratic token/resolution relationship, clarifies the "512" memory

Ran `encode_video_chunk` (via the real `Cosmos-Tokenizer-DV8x16x16` encoder, on GH200) across `target_size` in {160,224,256,320,384,448,512,640,720}, first on synthetic non-square noise frames, then on a real 8-frame chunk pulled from `videos/good.mp4` (2 women walking, native 2560x1440, 5.1s clip -- picked over the longer `boxing.mp4`/`martial_art.mp4` specifically because the user asked for "a few seconds only"). Token count follows `(target_size/16)^2 * 2` exactly (2 temporal codes per 8-frame chunk, confirmed independently in §3 below): 160->200, 224->392, 256->512, 320->800, 384->1152, 448->1568, 512->2048, 640->3200, 720->4050. Max token id stayed within the already-registered `<cosmos_0>`-`<cosmos_63999>` vocab range at every size tested -- resolution only changes token *count*, not the id range needed.

**Resolves the user's "wasn't cosmos already 512?" question:** not cosmos itself -- `data_prep/omnivideo_100k/step_a/step_a_tokenize_video.py`'s `EXTRACT_SIZE = 512` (matching `Seed2Tokenizer`'s own 512 default) is the **frame-extraction** resolution, upstream of cosmos. `CosmosVideoTokenizer.encode_video_chunk()` silently re-downsampled that 512 down to 160 (now 256) internally -- the 512 the user remembered never actually reached the Cosmos tokenizer.

Built `tools/cosmos_resolution_experiment.py`: pulls one real 8-frame chunk from a given video/start-frame, encodes+decodes it at several `target_size` values, and saves each reconstruction as its own mp4 alongside the original (upscaled for fair viewing) -- first run (`samples/cosmos_resolution_experiment/`) sent to the user directly (8 files: 1 reference + 7 resolutions) for a real visual pick, rather than judging off synthetic noise (which only proved the code runs, not that quality is acceptable).

**Bug caught and fixed while building this:** the script's `os.chdir(PROTOTYPE_DIR)` (needed to make `cosmos_tokenizer`'s local-package imports resolve) ran before an output path was made absolute, so the first attempt silently wrote outputs into `/e/.../prototype/samples/...` instead of the intended `3d-human-pose/samples/...` -- the exact same class of bug already fixed once in `decode_seed2.py` (§32). Fixed by resolving `--output-dir`/`--video` to absolute paths at the very top of `main()`, before any `chdir`.

### 2. SNAC L2 A/B on 3 real roleplay records -- confirms token cost, but decode quality judged by the user as clearly better

Wrote `tools/snac_l2_experiment.py`: pulls real rows directly from `laion/emotional-roleplay-finetuning-dataset`'s source parquet (`/p/data1/mmlaion/shared/vla/laion_emotional_roleplay/data/`), decodes the real mp3 audio, encodes with SNAC's full 3-level codebook (not just the production L0+L1 "listen" format), and saves 3 files per record: the real original audio, a `decoded_listen_L0L1` reconstruction (current production, L2 zero-filled), and a `decoded_speak_L0L1L2` reconstruction (real L2). Ran on 3 real records (`cv_Fairy-2__b1_13_Astonishment_Surprise_1`, `cv_Fairy-2__b1_18_Teasing_1`, `cv_Fairy-2__b1_07_Infatuation_1`):

| record | listen tokens | speak tokens | increase |
|---|---|---|---|
| b1_13_Astonishment_Surprise_1 | 342 | 798 | +133.3% |
| b1_18_Teasing_1 | 339 | 791 | +133.3% |
| b1_07_Infatuation_1 | 327 | 763 | +133.3% |

Exactly matches theory: L2 adds 4 tokens/base-frame on top of the existing 3 (L0+L1a+L1b), so +4/3 = +133.3% regardless of clip content. 2 of the 3 records' full 3-file sets were sent to the user directly for listening (6 files) rather than describing quality in words. **User's verdict after listening: L2 is clearly better ("L2 ngon lắm"), decided to move forward with adding it** -- a real quality/cost tradeoff decision made by ear, not guessed.

**MV-Omni constraint found while discussing this:** the user asked whether MV-Omni's SNAC data would also need re-encoding with L2. Checked directly: `mixture-vitae/MixtureVitae-Omni`'s real HF file listing (via `HfApi.list_repo_files`) has **only** `data/data/valid_snac_*.jsonl.gz` / `snac_emo_seed_0.jsonl.gz` -- no raw audio anywhere in the repo (`avc_tokenizer/`, `data/`, `jpeg_tokenizer/` are the only top-level dirs). Streamed a partial sample of `valid_snac_5.jsonl.gz` directly (HTTP Range request + streaming zlib decompress, no full download of the ~5.8GB compressed file) and confirmed the real token ids cycle through exactly the same 3 offset bands (L0 `128266-132361`, L1a `132362-136457`, L1b `144650-148745`) as our own `tokenize_snac.py` -- **MV-Omni's SNAC is also listen-format-only (L0+L1), and since no raw audio was ever published, L2 cannot be added to it after the fact** -- the fine detail was discarded at their encoding time and is unrecoverable from tokens alone. User's own resolution: will ask Huu directly for MV-Omni's source audio to re-tokenize it independently rather than treating this as blocked.

### 3. Cosmos square-crop removed -- root-caused to a robot-camera-era default that no longer fits omni scope

User's own realization, independently before I raised it: cosmos output looks "unnatural" on faces/signage because the pipeline center-crops to a square, and this was inherited from when the project's camera source was a robot's jittery, roughly-square camera -- not appropriate now that the pipeline ingests ordinary 16:9 YouTube video under the omni-modal scope.

Verified directly on the real encoder (not assumed) that `Cosmos-Tokenizer-DV8x16x16` accepts **non-square** input fine, as long as both H and W are multiples of 16 (the spatial compression factor) -- tested (256,256)/(256,448)/(256,464)/(288,512)/(270,480), all encoded successfully with the expected `(H/16)*(W/16)*2` token count.

Changed `encode_video_chunk()` (both `prototype/pipeline.py` and the git-tracked `pipeline_video/pipeline.py` mirror) from resize-shorter-side + `CenterCrop` (square, still cropping real content off the sides) to a pure aspect-preserving resize: shorter side = `target_size`, longer side rounded to the nearest multiple of 16, **no crop at all**. Token count is no longer `size^2`-based -- e.g. 256 shorter side on a 16:9 frame is now 256x448 = 896 tokens (2026-07-22, up from the 512 the square-crop version would have used at the same target_size, since the full width is now kept instead of being cropped away).

Re-ran `cosmos_resolution_experiment.py` (fixing its grid-shape assumption, which had hardcoded square grids -- updated to mirror the real encode logic) on the same `good.mp4` chunk at 4 resolutions (160/256/384/512) with the new aspect-preserving code -- all succeeded (160->360, 256->896, 384->2064, 512->3648 tokens) -- sent to the user as a second, v2 comparison set (`samples/cosmos_resolution_experiment_v2/`).

### 4. Frame-stride ("speed up video") experiment -- confirms zero token-cost impact, gives a concrete recommendation

Following up on the "8 frames too short" analysis from earlier this session (§35's predecessor conversation) and Huu's own "speed up the videos" suggestion: wrote `tools/cosmos_stride_experiment.py`, which samples every Nth native frame (stride 1/2/3/4) instead of every consecutive frame, keeping the input to Cosmos at exactly 8 frames either way. Ran on `videos/boxing.mp4` (continuous punching-bag motion, chosen over `good.mp4`'s slower walking pace specifically to make a stride effect visible), target_size=256 fixed so only stride varies:

| stride | real time/chunk | tokens |
|---|---|---|
| 1x (current) | 0.233s | 896 |
| 2x | 0.467s | 896 |
| 3x | 0.700s | 896 |
| 4x | 0.933s | 896 |

**Confirms the key property analytically predicted earlier: token count is completely flat across all strides (896 in every case)** -- Cosmos only counts input frames, not their spacing in the source video, so this is the only one of the fixes discussed today that changes real-time-per-chunk at zero token cost (unlike the resolution fix, which is quadratic). Sent all 8 files (original + decoded per stride) to the user for direct visual judgment.

**Recommendation given:** stride ~3 (~0.7s/chunk) as a starting point -- long enough to show a full punch-like motion unit, not so long that fast sub-motions between sampled frames go completely unseen (aliasing risk grows with stride). Framed generally (per the user's own point that 30fps shouldn't be hardcoded, since a future robot camera may run at a different native fps): define the target in **real seconds** (~0.7s) first, then derive `stride = round(native_fps * 0.7 / 8)` for whatever camera is actually in use, rather than hardcoding a frame count.

**Not yet resolved / flagged explicitly:** applying a stride-based speed-up to cosmos alone (without a matching change to how pose/agent windows are derived in Phase 3) would desync `cosmos_N` and `agent_N` from representing the same real timespan -- the exact `chunk_timing` alignment mechanism fixed 2026-07-20. Two options were laid out for the user to decide between: (a) also adjust Phase 3's windowing to match (a relatively cheap change -- it only regroups already-computed per-frame poses, not a re-run of the expensive HRNet/MotionBERT GPU stages, so may not actually violate the user's "pose pipeline is expensive, do video first" priority), or (b) widen cosmos's own input window while keeping external chunk_timing/interleave cadence at 8 frames (cosmos chunks would then overlap neighboring chunks' frame ranges -- an acceptable design pattern, but not literally "video pipeline only" if strict 1:1 sync with agent windows is required). Not decided yet.

### Status at end of entry

- Cosmos resolution choice: still open, pending the user reviewing both resolution comparison sets (v1 square, v2 aspect-preserving) just sent.
- Cosmos square-crop -> aspect-preserving: **done, verified on real non-square input**, applied to both pipeline copies.
- SNAC L2: **user has decided to move forward with it** based on real audio A/B -- not yet applied to the production `tokenize_snac.py` pipeline (that script still only encodes L0+L1); MV-Omni's own SNAC data cannot get L2 without new raw audio from Huu (user will ask directly).
